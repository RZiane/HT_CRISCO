...
def convTable(entry, tag, list_prpos, list_prfeat, list_upennpos, s_udpos):
    if entry[1] == tag[1]:
...
def process_conversion(inputfile, outputfile):
    ...
    for w in tqdm(root.findall('.//w')):
        ...
        s_token = s_token.replace('[', '').replace('(', '').replace(']', '').replace(')', '')
            ...
            if s_token in d_PRESTO.keys():
                ...
                if len(d_PRESTO[s_token]) != 1: # si plusieurs valeurs pour une entrée dans le dictionnaire alors ambiguité
                    for entry in d_PRESTO[s_token]: # itération des valeurs pour l'entrée
                        if entry[3] == s_lemma:
                            for tag in d_CorrTable[s_udpos]:
                                convTable(entry, tag, list_prpos, list_prfeat, list_upennpos, s_udpos)
                                                    
                if d_PRESTO[s_token][0][3] == s_lemma: # si une seule valeur pour l'entrée dans presto
                    for tag in d_CorrTable[s_udpos]:
                        convTable(d_PRESTO[s_token][0], tag, list_prpos, list_prfeat, list_upennpos, s_udpos)
